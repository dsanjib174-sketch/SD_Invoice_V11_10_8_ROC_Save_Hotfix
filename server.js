...
const db = new Database('sd_invoice_v11_10_8_roc_save_hotfix.db');
...
function ensureRateContractColumns(){
  const add = (col, type) => {
    try{
      const cols=db.prepare("PRAGMA table_info(rate_contracts)").all().map(x=>x.name);
      if(!cols.includes(col)) db.prepare(`ALTER TABLE rate_contracts ADD COLUMN ${col} ${type}`).run();
    }catch(e){}
  };
  ...
  add('hsn','TEXT');
  add('uom','TEXT');
  add('approved_rate','REAL DEFAULT 0');
  add('gst','REAL DEFAULT 0');
  add('status','TEXT DEFAULT "Active"');
  add('manual_rate_locked','TEXT');
  add('valid_from','TEXT');
  add('valid_to','TEXT');
  add('remarks','TEXT');
}
...
app.post('/api/universal/rate-contracts/save',auth,(req,res)=>{
  try{
    ensureRateContractColumns();
    const clientId = (typeof masterClientId==='function') ? masterClientId(req) : (req.user.clientId || null);
    if(!clientId) return res.status(400).json({error:'Client context missing. Please login again.'});

    const d=req.body||{};
    const customerId = Number(d.customer_id||0) || null;
    const productId = Number(d.product_id||0) || null;

    const cust = customerId ? db.prepare("SELECT * FROM customers WHERE id=? AND client_id=?").get(customerId,clientId) : null;
    const prod = productId ? db.prepare("SELECT * FROM products WHERE id=? AND client_id=?").get(productId,clientId) : null;

    const itemName = String(d.item_name || (prod && prod.name) || '').trim();
    if(!itemName) return res.status(400).json({error:'Item / Service Name is required'});
    const approvedRate = Number(d.approved_rate || d.rate || 0);
    if(approvedRate <= 0) return res.status(400).json({error:'Approved Rate must be greater than 0'});

    const row = {
      client_id: clientId,
      customer_id: customerId,
      product_id: productId,
      customer_name: String(d.customer_name || (cust && cust.name) || '').trim(),
      item_name: itemName,
      hsn: String(d.hsn || (prod && prod.hsn) || '').trim(),
      uom: String(d.uom || (prod && prod.unit) || 'Nos').trim(),
      approved_rate: approvedRate,
      gst: Number(d.gst || (prod && prod.gst) || 0),
      status: String(d.status || 'Active').trim(),
      manual_rate_locked: String(d.manual_rate_locked || d.manual || 'Manual Rate Locked').trim(),
      valid_from: String(d.valid_from || '').trim(),
      valid_to: String(d.valid_to || '').trim(),
      remarks: String(d.remarks || '').trim()
    };

    const cols=db.prepare("PRAGMA table_info(rate_contracts)").all().map(x=>x.name);
    const keys=Object.keys(row).filter(k=>cols.includes(k));
    const sql=`INSERT INTO rate_contracts (${keys.join(',')}) VALUES (${keys.map(()=>'?').join(',')})`;
    db.prepare(sql).run(...keys.map(k=>row[k]));

    res.json({success:true,message:'Rate Contract saved successfully'});
  }catch(err){
    console.error('ROC_SAVE_ERROR:', err && err.stack ? err.stack : err);
    res.status(500).json({error:'Rate Contract save failed: '+err.message});
  }
});
...
app.listen(PORT,()=>console.log('SD Invoice V11.10.8 ROC Save Hotfix'+PORT));
